...
variable "project_name" {
  description = "프로젝트명, 이벤트 브릿지, 스텝 함수를 이용한 배치 오케스트레이션"
  type        = string
  # 프로젝트명 수정
  default = "de-ai-16-eb-step-pipeline"
}
...
variable "glue_worker_type" {
  description = "Glue Worker의 인스턴스의 유형"
  type        = string
  default     = "G.1X"
}
variable "glue_number_of_workers" {
  description = "Glue Worker를 몇 개 가동"
  type        = number
  default     = 2
}

# 성공/실패 -> 메일로 받을 수 있도록 선택 저장
