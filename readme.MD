...
Python 로그 생성기 ┤
                  └─ sensor_text.log (비정형 데이터)
                         │
                         ▼
                    Fluent Bit <- 컨셉상 2가지 방향성 설계한 것임(실제는 1개만 수행)
                    /         \
                   /           \
          JSON 직결             TEXT
             │                  │
             ...
             │                  │
             ▼                  ▼
 factory-json-topic      factory-text-topic <- 토픽 : 카프카에서 메시지 구분하는 용도 
             \                 /
              \               /
...
```
  ...
  sh opt/kafka/bin/kafka-topics.sh --create --topic factory-json-topic --bootstrap-server 127.0.0.1:9092 --partitions 1 --replication-factor 1
